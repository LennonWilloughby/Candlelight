...
public class AIZombie : MonoBehaviour {
	...
	// Public
	public float senseDistance = 5.0f; // max distance from player that the enemy will react
	public float lightDistance = 10.0f; // max distance from a light source that the enemy will react
	...
	// Private
	private float moveSpeed = 4.0f; // enemy movement speed
	...
	void Update () {
		...
		if (currentDistance < senseDistance && LightSource.lightState == false){
			// If enemy hasn't reached player position yet
			if (currentDistance > rangeBetween)
				MoveToPlayer ();
		}
